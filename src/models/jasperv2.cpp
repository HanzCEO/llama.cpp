...
void llama_model_jasperv2::load_arch_hparams(llama_model_loader & ml) {
    ...
    // model type detection (same as Qwen3)
...
void llama_model_jasperv2::load_arch_tensors(llama_model_loader &) {
    LLAMA_LOAD_LOCALS;
    ...
    // output projection (linear_1 -> 2048)
    dense_2_out_layers   = create_tensor(tn(LLM_TENSOR_DENSE_2_OUT, "weight"), {n_embd, hparams.dense_2_feat_out}, TENSOR_NOT_REQUIRED);
    dense_2_out_layers_b = create_tensor(tn(LLM_TENSOR_DENSE_2_OUT, "bias"),   {hparams.dense_2_feat_out},         TENSOR_NOT_REQUIRED);
...
std::unique_ptr<llm_graph_context> llama_model_jasperv2::build_arch_graph(const llm_graph_params & params) const {
    return std::make_unique<graph>(*this, params);
}

llama_model_jasperv2::graph::graph(const llama_model & model, const llm_graph_params & params) :
